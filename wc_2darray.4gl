...
PUBLIC FUNCTION cell_class_append(x,y,c)
DEFINE x,y INTEGER
DEFINE c STRING
    LET cellArray[x,y].class = SFMT("%1 %2", cellArray[x,y].class,c)
END FUNCTION
...
FUNCTION get_click(s)
DEFINE s STRING
DEFINE x,y INTEGER
DEFINE pos1, pos2 INTEGER

    LET pos1 = s.getIndexOf("_",1)
    LET pos2 = s.getIndexOf("_",pos1+1)
    CASE 
        WHEN s MATCHES "cell*" 
            RETURN "cell", s.subString(pos1+1,pos2-1), s.subString(pos2+1, s.getLength())
        WHEN s MATCHES "row*"
            RETURN "row", NULL, s.subString(pos1+1, s.getLength())
        WHEN s MATCHES "col*"
            RETURN "col", s.subString(pos1+1, s.getLength()), NULL
        OTHERWISE
